# Tutorials

[Documentation](../README.md) > Tutorials

---

## Overview

Step-by-step guides for using and extending SpiritStream, from installation to advanced configuration.

## Documents

| Tutorial | Description | Time | Level |
|----------|-------------|------|-------|
| [01. Getting Started](./01-getting-started.md) | Installation on all platforms | 15 min | Beginner |
| [02. First Stream](./02-first-stream.md) | Basic streaming setup | 20 min | Beginner |
| [03. Multi-Platform](./03-multi-platform.md) | Stream to multiple services | 30 min | Intermediate |
| [04. Custom Encoding](./04-custom-encoding.md) | Advanced encoding configuration | 45 min | Advanced |
| [05. Contributing](./05-contributing.md) | Development setup and code style | 60 min | Advanced |

## Learning Path

```
Beginner
   ├── Getting Started → First Stream
   │
Intermediate
   ├── Multi-Platform Streaming
   │
Advanced
   ├── Custom Encoding → Contributing
```

## Prerequisites

- **Getting Started**: No prerequisites
- **First Stream**: Completed Getting Started
- **Multi-Platform**: Basic streaming knowledge
- **Custom Encoding**: Understanding of video codecs
- **Contributing**: Rust and TypeScript experience

---

*Section: 06-tutorials*

