 ...
 */
export async function setupMainWindowCloseHandler() {
  if (!isTauri()) {
  ...
  try {
    ...
    mainWindow.onCloseRequested(async () => {
      // Close the chat overlay before main window closes
      await closeChatOverlay();
    });
  } catch (error) {
    console.error('Failed to set up main window close handler:', error);
  }
}
