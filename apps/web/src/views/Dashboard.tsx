...
export function Dashboard({ onNavigate, onOpenProfileModal, onOpenTargetModal }: DashboardProps) {
  ...
  // Test stream configuration with real RTMP connectivity checks
  const handleTestStream = async () => {
    ...
    try {
      ...
      toast.info(t('toast.testingConnectivity', 'Testing connectivity to {{count}} targets...', { count: targetCount }));

      const connectivityResult = await testStreamConnectivity(
        currentProfile,
        { enabledTargetsOnly: false },
        (_current, _total, targetName, result) => {
          // Update UI with current target being tested
          setTestingTarget(targetName);

          // Show individual result when available
          if (result) {
            if (result.success) {
              toast.success(
                t('toast.targetTestPassed', '✓ {{name}}: Connected ({{ms}}ms)', {
              ...
              toast.error(
                t('toast.targetTestFailed', '✗ {{name}}: {{message}}', {
                  name: result.targetName,
      ...
      if (connectivityResult.allPassed) {
        toast.success(
          t('toast.allTestsPassed', 'All {{count}} targets passed connectivity test!', {
            count: connectivityResult.passed,
          })
        );
      } else if (connectivityResult.passed > 0) {
        toast.info(
          t('toast.someTestsFailed', '{{passed}} passed, {{failed}} failed', {
            passed: connectivityResult.passed,
            failed: connectivityResult.failed,
          })
        );
      } else {
        toast.error(
          t('toast.allTestsFailed', 'All {{count}} targets failed connectivity test', {
            count: connectivityResult.failed,
          })
        );
      }
    } catch (err) {
