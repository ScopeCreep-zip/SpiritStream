...
pub struct AuditLogQuery {
    ...
    /// (panic_triggered, pii_filter_fired, oauth_refresh, …).
    #[serde(default)]
    kind: Option<String>,
}
...
fn action_kind_str(action: &spiritstream_core::services::AuditAction) -> &'static str {
    ...
    match action {
        ...
        PiiFilterFired { .. } => "pii_filter_fired",
...
pub async fn v1_chat_send_proxy(
    ...
    // PII filter runs server-side on outbound messages.
    // The active profile's blocklist + fuzzy flag are cached on
    // AppState by set_active_profile, so this is a single mutex read.
    {
        let snapshot = state.active_profile_pii.lock().await.clone();
        if let Some((blocklist, fuzzy)) = snapshot {
            if !blocklist.is_empty() {
                let mode = if fuzzy {
                    spiritstream_core::services::PiiMatchMode::Fuzzy
                } else {
                    spiritstream_core::services::PiiMatchMode::Strict
                };
                if let spiritstream_core::services::PiiCheck::Match { phrase_id } =
                    spiritstream_core::services::pii_filter::check(&trimmed, &blocklist, mode)
                {
                    // Audit each targeted platform; emit event for UI banner.
                    for platform in &targets {
                        let _ = state.audit.record(
                            spiritstream_core::services::AuditAction::PiiFilterFired {
                    ...
                    state.event_bus.emit(
                        ...
                        serde_json::json!({
                            "phraseId": phrase_id,
                            "platformCount": targets.len(),
                        }),
                    );
                    return Err(spiritstream_core::CoreError::ChatBlockedByPii { phrase_id }.into());
                }
            }
        }
    }
    ...
    let results = state
        .chat_manager
        .send_message(trimmed.clone(), &targets)
        .await;
