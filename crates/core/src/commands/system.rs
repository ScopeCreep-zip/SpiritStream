...
pub fn get_encoders() -> Result<Encoders, CoreError> {
    use crate::models::{EncoderKind, EncoderMeta};
    ...
    // Build per-encoder metadata (kind + preset family). Frontend uses
    // this to render the hardware/software badge and to pick the
    // preset list for the codec the user selected, without local
    // substring matching.
