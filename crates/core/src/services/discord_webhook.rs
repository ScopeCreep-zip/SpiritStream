...
struct RateLimitResponse {
    ...
    retry_after: f64,
    /// Whether this is a global rate limit (true) or per-route (false).
    /// Discord returns this on 429; operators care because a global cap
    /// means *every* webhook send is blocked for `retry_after`, not just
    /// this one URL — useful signal in support tickets.
    global: bool,
}
