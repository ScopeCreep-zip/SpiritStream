...
impl TwitchConnector {
    ...
    /// Render the `badges` field of a Privmsg into the `name/version`
    /// format Twitch's IRC tags use (e.g. `subscriber/24`,
    /// `moderator/1`). `twitch-irc 5.x` exposes badges directly on
    /// `PrivmsgMessage`, so no raw-tag parsing is needed.
    fn parse_badges(badges: &[twitch_irc::message::Badge]) -> Option<Vec<String>> {
        if badges.is_empty() {
            return None;
        }
        Some(
            ...
            while let Some(message) = incoming_messages.recv().await {
                match message {
                    ServerMessage::Privmsg(msg) => {
                        ...
                        if let Some(badges) = Self::parse_badges(&msg.badges) {
