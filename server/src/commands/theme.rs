...
use crate::services::validate_extension;
use serde_json::{json, Value};
use std::path::PathBuf;

pub async fn handle(state: &AppState, command: &str, payload: &Value) -> Option<Result<Value, String>> {
    match command {
        "list_themes" => Some(list_themes(state)),
        "refresh_themes" => Some(refresh_themes(state)),
        "get_theme_tokens" => Some(get_theme_tokens(state, payload)),
        "install_theme" => Some(install_theme(state, payload)),
        _ => None,
    }
}

fn list_themes(state: &AppState) -> Result<Value, String> {
...
fn refresh_themes(state: &AppState) -> Result<Value, String> {
...
fn get_theme_tokens(state: &AppState, payload: &Value) -> Result<Value, String> {
...
fn install_theme(state: &AppState, payload: &Value) -> Result<Value, String> {
